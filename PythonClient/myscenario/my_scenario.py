...
def get_lidar_data(vehicle_name):
    ...
    with client_lock:
        lidarData = client.getLidarData(vehicle_name=vehicle_name)
...
def follower_loop():
    ...
    while True:
        ...
        target_pos = leader_pos + np.array([0.0, 5.0, 0.0]) 
        ...
        if detect_obstacles(lidar_points, 10.0):
            ...
            perform_avoidance(drone_pos, "follower")
            with client_lock:
                client.moveByVelocityAsync(0, 0, 0, 1, vehicle_name="follower").join()  
            perform_avoidance(drone_pos, "follower")
        else:
            move_towards_target(drone_pos, target_pos, "follower")
